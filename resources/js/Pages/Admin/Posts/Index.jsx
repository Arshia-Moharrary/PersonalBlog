...
export default function Index({ posts }) {
    ...
    const confirmDelete = () => {
        destroy(route(`admin.${settings.route}.destroy`, postToDelete), {
            preserveScroll: true,
        });
    };
                        ...
                        {postList.map(post => (
                            <tr key={post.id} className="border-t">
                                ...
                                <td className="px-4 py-2 flex gap-2">
                                    ...
                                    {settings.hasDelete && (
                                        <button onClick={() => handleDelete(post.id)} className="text-red-600 hover:underline flex items-center gap-1 text-xs">
